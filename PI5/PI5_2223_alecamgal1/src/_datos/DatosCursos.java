...
public class DatosCursos {
	...
	//Metodo que devuelve todas las  tematicas de un curso
		public static List<Integer> getTematicasCurso(Integer i){
			Set<Integer> res = Set2.empty();
			Curso c = getCursos().get(i);
			res.addAll(c.tematicas());
			return List2.ofCollection(res);
		}
	...
	public static Integer trataTematica(Integer i, Integer j) {
		Integer res = 0;
		List<Integer> tematicas = getTematicas();
		if (cursos.get(i).tematicas().contains(tematicas.get(j-1))) {
			res = 1;
		}
		return res;
	}
	...
	//Metodo que devuelve el centro de un curso
